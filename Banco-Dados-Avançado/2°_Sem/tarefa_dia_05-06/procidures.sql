...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        -- SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Erro durante a transferência.';
    END;

    START TRANSACTION;
	IF p_valor > 0 THEN
    SELECT saldo INTO saldo_origem FROM contas WHERE id_conta = p_id_conta_origem FOR UPDATE;
...
SELECT * FROM contas WHERE id_conta in (1,2);
